...
<div class="col-md-12">
    ...
    <div class="card">
                                ...
                                <div class="card-body">
                                    <ul class="list-group list-group-flush">
                                        <li class="list-group-item">
                                            <a href="<?= base_url('guru/nilai/' . $sk['id_sub_kursus'] . '/' . $sk['id_kursus']) ?>" class="btn btn-secondary btn-flat btn-xs"><i class="fa fa-eye"></i> &nbsp;Penilaian</a>
                                            <a href="<?= base_url('guru/absensiSiswa/' . $sk['id_sub_kursus'] . '/' . $sk['id_kursus']) ?>" class="btn bg-cyan btn-flat btn-xs"><i class="fa fa-eye"></i> &nbsp;Absensi Siswa</a>
                                            <a href="<?= base_url('guru/materi/' . $sk['id_sub_kursus'] . '/' . $sk['id_kursus']) ?>" class="btn btn-warning btn-flat btn-xs"><i class="fa fa-eye"></i> &nbsp;Materi</a>
                                            <a href="<?= base_url('guru/kuis/' . $sk['id_sub_kursus'] . '/' . $sk['id_kursus']) ?>" class="btn bg-lightblue btn-flat btn-xs"><i class="fa fa-eye"></i> &nbsp;Kuis</a>
        ...
        <div class="card-footer">
            <a href="<?= base_url('guru/kursus') ?>" class="btn btn-sm btn-danger btn-block btn-flat">Kembali</a>
        </div>
    </div>
</div>
...
<script>
    ...
    function tambah(id_kursus) {
        $.ajax({
            type: "post",
            url: "<?= site_url('guru/formTambahSubKursus') ?>",
            data: {
                id_kursus: id_kursus
            },
            dataType: "json",
    ...
    function edit(id_sub_kursus) {
        $.ajax({
            type: "post",
            url: "<?= site_url('guru/formEditSubKursus') ?>",
            data: {
    ...
    function hapus(id_sub_kursus) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: "post",
                    url: "<?= site_url('guru/hapusSubKursus') ?>",
                    data: {
                        id_sub_kursus: id_sub_kursus,
                    },
                    dataType: "json",
