<?= form_open('admin/simpanDataMapelBanyak', ['class' => 'formSimpanBanyak']) ?>
<?= csrf_field() ?>
<table class="table table-bordered text-nowrap">
    ...
    <tbody class="formTambah">
        <tr>
            <td>
                <input type="text" name="mapel[]" class="form-control form-control-sm" style="border-radius: 0px;" placeholder="Masukan Matapelajaran" required>
            </td>
                    ...
                    if (response.success) {
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                window.location.href = ("<?= site_url('admin/mapel') ?>")
                            }
                        })
